...
- **Dataset Size:** 100 User Queries (Curated manually against Vietnamese Law base).
- **Ground Truth:** Sourced directly from official legal documentation.

| Metric | Threshold (Pass Mark) | Description |
|--------|-----------------------|-------------|
| **Hit@5** | `≥ 0.85` | Percentage of queries where the correct document appears in the top 5. |
| **MRR@5** | `≥ 0.75` | Evaluates how high the correct legal document is ranked in the top 5 results. |
| **Faithfulness** | `≥ 0.90` | Measures if the generated answer is strictly grounded in the retrieved context (0 hallucination). |
| **Answer Relevancy** | `≥ 0.85` | Measures the semantic similarity between the user's question and the generated answer. |

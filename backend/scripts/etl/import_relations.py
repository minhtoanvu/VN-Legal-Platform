# Import quan hệ giữa các văn bản từ dataset relationships của HuggingFace
# Cách này khác import_phapdien_relations.py ở chỗ: dùng split 'relationships' có sẵn trong dataset
# thay vì tự parse từ related_note_text
...
# Map tên quan hệ tiếng Việt sang loại chuẩn trong DB
RELATION_MAP = {
    'can cu': 'CITES',
    'huong dan thi hanh': 'GUIDES',
    'huong dan': 'GUIDES',
    'thay the': 'REPLACES',
    'sua doi, bo sung': 'AMENDS',
    'sua doi': 'AMENDS',
    'bo sung': 'AMENDS',
    'bai bo': 'REVOKES',
    'dinh chi': 'REVOKES',
...
async def import_relations():
    ...
    # Bước 1: lấy mapping id -> so_ky_hieu từ metadata split
    ...
    # Bước 2: lấy relationships từ split tương ứng
    ...
    # Bước 3: map sang UUID trong DB và insert
    async with AsyncSessionLocal() as db:
        ...
        if not matched:
            print('Khong co match nao. Kiem tra lai key mapping:')
            print('  th1nhng0 so_ky_hieu mau:', list(id_to_sokyhieu.values())[:10])
            print('  DB doc_number mau:', list(doc_map.keys())[:10])
            return
        ...
        for i in range(0, len(matched), BATCH):
            ...
            values = ','.join(
                f"(gen_random_uuid(), '{src}', '{tgt}', '{rel}', NOW())"
                for src, tgt, rel in batch
            )
            await db.execute(text(
                f'INSERT INTO document_relations (id, source_doc_id, target_doc_id, relation_type, created_at) '
